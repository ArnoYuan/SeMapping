...
Map<Cell, Storage, isClass>::Map(const Point& center, double worldSizeX,
		...
		m_storage((int) ceil(worldSizeX / delta),
				(int) ceil(worldSizeY / delta)) {
	m_center = center;
...
//FIXME size the map so that m_center will be setted accordingly
	...
	Map<double, DoubleArray2D, false>* plainMap = new Map<double, DoubleArray2D,
			false>(center, (pmax - pmin).x, (pmax - pmin).y, getDelta());
	for (int x = 0; x < getMapSizeX() - 1; x++)
		for (int y = 0; y < getMapSizeY() - 1; y++) {
			IntPoint p(x, y);
