...
void EntropyRecord::write(ostream& os) {
	os << setiosflags(ios::fixed) << setprecision(6) << "ENTROPY "
			<< poseEntropy << " " << trajectoryEntropy << " " << mapEntropy;
	os << " " << time << " pippo " << time << endl;
}
...
double RecordList::getLogWeight(unsigned int i,
		RecordList::const_iterator frame) const {
	double weight = 0;
	unsigned int currentIndex = i;
...
void RecordList::printLastParticles(ostream& os) const {
	...
	for (vector<OrientedPoint>::const_iterator it = scanmatch->poses.begin();
		...
		os << "MARKER [color=black; circle=" << it->x * 100 << ","
				<< it->y * 100 << ",10] 0 pippo 0" << endl;
	}
}
...
RecordList RecordList::computePath(unsigned int i,
		RecordList::const_iterator frame) const {
	unsigned int currentIndex = i;
	...
	bool first = true;
	for (RecordList::const_reverse_iterator it(frame); it != rend(); it++) {
		const ScanMatchRecord* scanmatch =
				dynamic_cast<const ScanMatchRecord*>(*it);
		if (scanmatch) {
		...
		const ResampleRecord* resample =
				dynamic_cast<const ResampleRecord*>(*it);
		if (resample) {
			currentIndex = resample->indexes[currentIndex];
		}
	}
...
void RecordList::printPath(ostream& os, unsigned int i, bool err,
		bool rawodom) const {
	unsigned int currentIndex = i;
	OrientedPoint p(0, 0, 0);
	RecordList rl;
	...
	for (RecordList::const_reverse_iterator it = rbegin(); it != rend(); it++) {
		...
		const OdometryRecord* odometry =
				dynamic_cast<const OdometryRecord*>(*it);
		if (odometry) {
			PoseRecord* pose = new PoseRecord;
			pose->dim = 0;
		...
		const RawOdometryRecord* rawodometry =
				dynamic_cast<const RawOdometryRecord*>(*it);
		if (rawodometry) {
			PoseRecord* pose = new PoseRecord;
			pose->dim = 0;
	...
	for (RecordList::iterator it = rl.begin(); it != rl.end(); it++) {
		...
		started =
				started || dynamic_cast<const LaserRecord*>(*it) ? true : false;
		...
		if (computedTransformation) {
			...
			if (pos) {
				if (pos->truePos) {
					...
					if (tpf) {
						tpf = false;
						OrientedPoint realDelta = absoluteDifference(pos->pose,
								realStart);
						OrientedPoint trueDelta = absoluteDifference(truePose,
								trueStart);
