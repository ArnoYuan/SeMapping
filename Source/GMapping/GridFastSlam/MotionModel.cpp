...
namespace NS_GMapping
{
  ...
  {
    OrientedPoint n(p);
    double lm = linearMove + fabs(linearMove) * sampleGaussian(srr) + fabs(
        angularMove) * sampleGaussian(str);
    double am = angularMove + fabs(linearMove) * sampleGaussian(srt) + fabs(
        angularMove) * sampleGaussian(stt);
                                            ...
                                            const OrientedPoint& pold) const
  {
    ...
    noisypoint.theta += sampleGaussian(
        stt * fabs(delta.theta) + srt * sqrt(
