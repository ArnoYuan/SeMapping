...
inline void GridSlamProcessor::scanMatch(const double* plainReading) {
	...
	for (ParticleVector::iterator it = m_particles.begin();
		...
		if (score > m_minimumScore) {
			...
			if (m_infoStream) {
				m_infoStream
						<< "Scan Matching Failed, using odometry. Likelihood="
						<< l << std::endl;
				m_infoStream << "lp:" << m_lastPartPose.x << " "
						<< m_lastPartPose.y << " " << m_lastPartPose.theta
						<< std::endl;
...
inline bool GridSlamProcessor::resample(const double* plainReading,
		int adaptSize, const RangeReading* reading) {
	...
	if (m_neff < m_resampleThreshold * m_particles.size()) {
		...
		if (m_outputStream.is_open()) {
			...
			for (std::vector<unsigned int>::const_iterator it =
					m_indexes.begin(); it != m_indexes.end(); it++) {
				m_outputStream << *it << " ";
			}
