#ifndef _GFSREADER_H_
#define _GFSREADER_H_
...
namespace NS_GMapping
{
  ...
  namespace NS_GFSReader
  {
    ...
    struct RecordList: public list< Record* >
    {
      ...
      read(istream& is);
      double
      getLogWeight(unsigned int i) const;
      double
      getLogWeight(unsigned int i, RecordList::const_iterator frame) const;
      unsigned int
      getBestIdx() const;
      void
      printLastParticles(ostream& os) const;
      void
      printPath(ostream& os, unsigned int i, bool err = false,
                bool rawodom = false) const;
      RecordList
      computePath(unsigned int i, RecordList::const_iterator frame) const;
      void
      destroyReferences();
    };
