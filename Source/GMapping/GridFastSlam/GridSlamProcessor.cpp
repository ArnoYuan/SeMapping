...
namespace NS_GMapping
{
      ...
      : last_update_time_(0.0), m_particles(gsp.m_particles), m_infoStream(cout)
  {
    ...
    cout << "m_odoPose=" << m_odoPose.x << " " << m_odoPose.y << " " << m_odoPose.theta << endl;
    cout << "m_lastPartPose=" << m_lastPartPose.x << " " << m_lastPartPose.y << " " << m_lastPartPose.theta << endl;
    ...
    cout << "Tree: normalizing, resetting and propagating weights within copy construction/cloneing ...";
...
//HERE STARTS THE BEEF
  ...
  GridSlamProcessor::Particle::Particle(const ScanMatcherMap& m)
      : map(m), pose(0, 0, 0), weight(0), weightSum(0), gweight(0),
        previousIndex(0)
  {
    node = 0;
  }
                                      ...
                                      int adaptParticles)
  {
    ...
    {
      cout << "***********************************************************************" << endl;
      cout << "********** Error: m_distanceThresholdCheck overridden!!!! *************" << endl;
      cout << "m_distanceThresholdCheck=" << m_distanceThresholdCheck << endl;
      cout << "Old Odometry Pose= " << m_odoPose.x << " " << m_odoPose.y << " " << m_odoPose.theta << endl;
      cout << "New Odometry Pose (reported from observation)= " << relPose.x << " " << relPose.y << " " << relPose.theta << endl;
      cout << "***********************************************************************" << endl;
      cout << "** The Odometry has a big jump here. This is probably a bug in the   **" << endl;
      cout << "** odometry/laser input. We continue now, but the result is probably **" << endl;
      cout << "** crap or can lead to a core dump since the map doesn't fit.... C&G **" << endl;
      cout << "***********************************************************************" << endl;
    }
    ...
    if(!m_count || m_linearDistance >= m_linearThresholdDistance || m_angularDistance >= m_angularThresholdDistance || (period_ >= 0.0 && (reading.getTime() - last_update_time_) > period_))
    {
      ...
      cout << "Laser Pose= " << reading.getPose().x << " " << reading.getPose().y << " " << reading.getPose().theta << endl;
  ...
  std::ofstream&
  GridSlamProcessor::outputStream()
  {
    return m_outputStream;
  }

  std::ostream&
  GridSlamProcessor::infoStream()
  {
    return m_infoStream;
  }
