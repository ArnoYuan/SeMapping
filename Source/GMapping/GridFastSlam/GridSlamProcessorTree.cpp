...
//BEGIN State Save/Restore
  ...
  GridSlamProcessor::TNodeVector GridSlamProcessor::getTrajectories() const
  {
    ...
    while(!border.empty())
    {
      ...
      pair< TNodeMultimap::iterator, TNodeMultimap::iterator > p = parentCache.equal_range(
          node);
      double childs = 0;
  ...
  void GridSlamProcessor::integrateScanSequence(GridSlamProcessor::TNode* node)
  {
    ...
    while(aux != 0)
    {
          ...
          it != m_particles.end(); it++)
      {
        ...
        double s = sin(oldPose.theta - it->pose.theta), c = cos(
            oldPose.theta - it->pose.theta);
...
//BEGIN
  ...
  double GridSlamProcessor::propagateWeights()
  {
    ...
    {
      cout << "ERROR: ";
      cout << "root->accWeight=" << lastNodeWeight << "    sum_leaf_weights=" << aw << endl;
      assert(0);
    }
