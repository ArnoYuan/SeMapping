...
ScanMatcherProcessor::ScanMatcherProcessor(double xmin, double ymin,
		double xmax, double ymax, double delta, double patchdelta) :
		m_map(Point((xmax + xmin) * .5, (ymax + ymin) * .5), xmax - xmin,
				ymax - ymin, patchdelta), m_pose(0, 0, 0) {
	m_regScore = 300;
	m_critScore = .5 * m_regScore;
	m_maxMove = 1;
	m_beams = 0;
...
void ScanMatcherProcessor::processScan(const RangeReading & reading) {
	...
	if (m_count) {
		if (m_computeCovariance) {
			...
			score = m_matcher.optimize(newPose, cov, m_map, m_pose,
					plainReading);
			/*
			...
			if (useICP) {
				cout << "USING ICP" << endl;
				score = m_matcher.icpOptimize(newPose, m_map, m_pose,
						plainReading);
			} else
				score = m_matcher.optimize(newPose, m_map, m_pose,
						plainReading);
		}
