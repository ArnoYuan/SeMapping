...
double icpStep(OrientedPoint & retval, const PointPairContainer& container) {
	...
	int size = 0;
	for (ContainerIterator it = container.begin(); it != container.end();
			it++) {
	...
	for (ContainerIterator it = container.begin(); it != container.end();
			it++) {
		PointPair mf = std::make_pair(it->first - mean.first,
				it->second - mean.second);
...
double icpNonlinearStep(OrientedPoint & retval,
	...
	for (ContainerIterator it = container.begin(); it != container.end();
		...
		PointPair mf = std::make_pair(it->first - mean.first,
				it->second - mean.second);
		double dalpha = atan2(mf.second.y, mf.second.x)
				- atan2(mf.first.y, mf.first.x);
