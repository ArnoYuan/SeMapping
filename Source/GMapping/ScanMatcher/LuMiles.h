...
class LuMilesProcessor {
	...
	step(const PointVector& src, const PointVector& dest);
};

OrientedPoint LuMilesProcessors::step(const PointVector& src,
	...
	for (PointVector::const_iterator it = dest.begin(); it != dest.end();
			it++) {
		dmx += it->x;
		dmy += it->y;
	}
