...
GMappingApplication::GMappingApplication() {
	...
	map_tf_srv = new NS_Service::Server<NS_ServiceType::ServiceTransform>(
			"ODOM_MAP_TF",
			boost::bind(&GMappingApplication::mapTransformService, this, _1));

	map_srv = new NS_Service::Server<NS_ServiceType::ServiceMap>("MAP",
			boost::bind(&GMappingApplication::mapService, this, _1));
...
*/
	...
	if (addScan(laser, odom_pose)) {
		...
		console.debug("new best pose: %.3f, %.3f, %.3f.", mpose.x, mpose.y,
				mpose.theta);
		console.debug("odom pose: %.3f, %.3f, %.3f.", odom_pose.x, odom_pose.y,
				odom_pose.theta);
		console.debug("correction: %.3f, %.3f, %.3f.", mpose.x - odom_pose.x,
		...
		if (!got_map
				|| (laser.header.stamp - last_map_update)
...
void GMappingApplication::mapService(NS_ServiceType::ServiceMap& srv_map) {
...
void GMappingApplication::mapTransformService(
...
double GMappingApplication::computePoseEntropy() {
	...
	for (std::vector<GridSlamProcessor::Particle>::const_iterator it =
			gsp->getParticles().begin(); it != gsp->getParticles().end();
			++it) {
	...
	for (std::vector<GridSlamProcessor::Particle>::const_iterator it =
		...
		if (it->weight / weight_total > 0.0)
			entropy += it->weight / weight_total
					* log(it->weight / weight_total);
	}
	return -entropy;
}
...
bool GMappingApplication::getOdomPose(OrientedPoint& gmap_pose) {
	...
	gmap_pose = OrientedPoint(base_odom_tf.transform.translation.x,
			base_odom_tf.transform.translation.y, yaw);
...
void GMappingApplication::updateMap(NS_DataType::LaserScan& laser_data) {
	...
	console.debug("  x [%.3f, %.3f] , y [%.3f, %.3f]:", xmin_, xmax_, ymin_,
			ymax_);
	...
	for (GridSlamProcessor::TNode* n = best.node; n; n = n->parent) {
		console.debug("[%d]  %.3f  %.3f  %.3f", node_count++, n->pose.x,
	...
	if (map.info.width != (unsigned int) smap.getMapSizeX()
		...
		console.message("Map position is from (%f, %f) to (%f, %f)", xmin_,
				ymin_, xmax_, ymax_);
		...
		console.message("Map origin: (%f, %f).", map.info.origin.position.x,
...
bool GMappingApplication::addScan(NS_DataType::LaserScan& laser_data,
	...
	if (do_reverse_range) {
		...
		for (int i = 0; i < num_ranges; i++) {
			if (laser_data.ranges[num_ranges - i - 1] < laser_data.range_min) {
				...
				ranges_double[i] =
						(double) laser_data.ranges[num_ranges - i - 1];
			}
		}
	} else {
		for (unsigned int i = 0; i < laser_data.ranges.size(); i++) {
