...
namespace NS_GMapping
{
  ...
  GMappingApplication::GMappingApplication()
  {
    ...
    map_srv = new NS_Service::Server< NS_ServiceType::ServiceMap >(
        "MAP", boost::bind(&GMappingApplication::mapService, this, _1));
  ...
  void GMappingApplication::updateMap(NS_DataType::LaserScan& laser_data)
  {
    ...
    if(map.info.width != (unsigned int)smap.getMapSizeX() || map.info.height != (unsigned int)smap.getMapSizeY())
    {
      ...
      console.message("Map size update!current is %d x %d.", smap.getMapSizeX(),
                      smap.getMapSizeY());
      console.message("Map position is from (%f, %f) to (%f, %f)", xmin_, ymin_,
                      xmax_, ymax_);
