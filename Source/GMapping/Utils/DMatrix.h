...
template<class X>
DMatrix<X>&
DMatrix<X>::operator=(const DMatrix& m) {
	if (!--(*shares)) {
		delete[] elems;
		delete[] mrows;
		delete shares;
		...
		for (int j = 0; j < nrows; j++)
			if (j != i) {
				...
				for (int l = 0; l < nrows; l++) {
					aux1.mrows[j][l] = aux1.mrows[j][l]
							- tmp * aux1.mrows[i][l];
					aux2.mrows[j][l] = aux2.mrows[j][l]
							- tmp * aux2.mrows[i][l];
				}
			}
	}
...
template<class X>
std::ostream&
operator<<(std::ostream& os, const DMatrix<X> &m) {
	os << "{";
