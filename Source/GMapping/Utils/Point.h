...
#define DEBUG_STREAM cout << __PRETTY_FUNCTION__ << ":" //FIXME
...
namespace NS_GMapping
{
  ...
  inline point< T > interpolate(const point< T >& p1, const F& t1,
                                const point< T >& p2, const F& t2, const F& t3)
  {
    F gain = (t3 - t1) / (t2 - t1);
                                           ...
                                           const F& t2, const F& t3)
  {
    ...
    double s = sin(p1.theta) + sin(p2.theta) * gain, c = cos(p1.theta) + cos(
        p2.theta) * gain;
    p.theta = atan2(s, c);
    return p;
  }
  ...
  template< class T, class A >
  inline double euclidianDist(const point< T >& p1,
                              const orientedpoint< T, A >& p2)
  {
    return hypot(p1.x - p2.x, p1.y - p2.y);
  }
