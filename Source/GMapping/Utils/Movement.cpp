...
namespace NS_GMapping
{
  ...
  {
    FSRMovement move;
    move.f = (pt2.y - pt1.y) * sin(pt1.theta) + (pt2.x - pt1.x) * cos(
        pt1.theta);
    move.s = +(pt2.y - pt1.y) * cos(pt1.theta) - (pt2.x - pt1.x) * sin(
        pt1.theta);
      ...
      const OrientedPoint& reference_pt_frame2, const OrientedPoint& pt_frame1)
  {
    ...
    FSRMovement tmp = composeMoves(composeMoves(trans_refp2, itrans_refp1),
                                   trans_pt);
