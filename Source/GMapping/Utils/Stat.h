...
double
sampleGaussian(double sigma, unsigned long int S = 0);

double
evalGaussian(double sigmaSquare, double delta);
double
evalLogGaussian(double sigmaSquare, double delta);
int
sampleUniformInt(int max);
double
sampleUniformDouble(double min, double max);

struct Covariance3 {
	Covariance3
	operator +(const Covariance3 & cov) const;
...
struct Gaussian3 {
	...
	Covariance3 cov;
	double
	eval(const OrientedPoint& p) const;
	void
	computeFromSamples(const std::vector<OrientedPoint> & poses);
	void
	computeFromSamples(const std::vector<OrientedPoint> & poses,
...
Gaussian3 computeGaussianFromSamples(PointIterator& pointBegin,
		PointIterator& pointEnd, WeightIterator& weightBegin,
