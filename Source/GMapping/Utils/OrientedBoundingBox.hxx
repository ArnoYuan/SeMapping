...
{
  return sqrt((ul.x - ll.x) * (ul.x - ll.x) + (ul.y - ll.y) * (ul.y - ll.y)) * sqrt(
      (ul.x - ur.x) * (ul.x - ur.x) + (ul.y - ur.y) * (ul.y - ur.y));
}

template< class NUMERIC >
OrientedBoundingBox< NUMERIC >::OrientedBoundingBox(
    std::vector< point< NUMERIC > > p)
{
  ...
  if((x3 == 0) || (x2 == 0) || (x4 * x4 - 2 * x1 * x4 + x1 * x1 + 4 * x2 * x3 < 0))
  {
    fprintf(
        stderr,
        "error computing the Eigenvectors (%s, line %d)\nx3=%lf, x2=%lf, term=%lf\n\n",
        __FILE__,
        __LINE__, x3, x2, (x4 * x4 - 2 * x1 * x4 + x1 * x1 + 4 * x2 * x3));
  ...
  // eigenvalues
  double lamda1 = 0.5 * (x4 + x1 + sqrt(
      x4 * x4 - 2.0 * x1 * x4 + x1 * x1 + 4.0 * x2 * x3));
  double lamda2 = 0.5 * (x4 + x1 - sqrt(
      x4 * x4 - 2.0 * x1 * x4 + x1 * x1 + 4.0 * x2 * x3));
