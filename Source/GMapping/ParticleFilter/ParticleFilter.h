...
void toLogForm(OutputIterator& out, const Iterator & begin,
...
struct uniform_resampler {
	...
	resampleIndexes(const std::vector<Particle> & particles,
			int nparticles = 0) const;
	std::vector<Particle>
	resample(const std::vector<Particle> & particles, int nparticles = 0) const;
	Numeric
	neff(const std::vector<Particle> & particles) const;
};
...
struct evolver {
	...
	void
	evolve(std::vector<Particle>& particles);
	void
	evolve(std::vector<Particle>& dest, const std::vector<Particle>& src);
};
...
void auxiliary_evolver<Particle, Numeric, QualificationModel, EvolutionModel,
	...
	for (typename std::vector<Particle>::const_iterator it = particles.begin();
			it != particles.end(); ++it, i++) {
		observationWeights[i] = likelyhoodModel.likelyhood(
				qualificationModel.evolve(*it));
	}
	uniform_resampler<Numeric, Numeric> resampler;
	std::vector<unsigned int> indexes(
			resampler.resampleIndexes(observationWeights));
	for (typename std::vector<unsigned int>::const_iterator it =
...
void auxiliary_evolver<Particle, Numeric, QualificationModel, EvolutionModel,
	...
	for (typename std::vector<unsigned int>::const_iterator it =
		...
		dest.back().weight *= likelyhoodModel.likelyhood(particle)
