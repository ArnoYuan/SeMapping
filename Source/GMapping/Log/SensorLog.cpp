...
SensorLog::SensorLog(const SensorMap& sm) :
		m_sensorMap(sm) {
}
...
OdometryReading*
SensorLog::parseOdometry(istream& is, const OdometrySensor* osen) const {
...
RangeReading*
SensorLog::parseRange(istream& is, const RangeSensor* rs) const {
	if (rs->newFormat) {
...
OrientedPoint SensorLog::boundingBox(double& xmin, double& ymin, double& xmax,
		double& ymax) const {
	xmin = ymin = 1e6;
	xmax = ymax = -1e6;
	bool first = true;
	...
	for (const_iterator it = begin(); it != end(); it++) {
		...
		const OdometryReading* odometry =
				dynamic_cast<const OdometryReading*>(reading);
		if (odometry) {
			lxmin = lxmax = odometry->getPose().x;
			lymin = lymax = odometry->getPose().y;
		}

		const RangeReading* rangeReading =
				dynamic_cast<const RangeReading*>(reading);
		if (rangeReading) {
