...
//SensorStream
SensorStream::SensorStream(const SensorMap& sensorMap) :
		m_sensorMap(sensorMap) {
}
...
SensorReading*
SensorStream::parseReading(std::istream& is, const SensorMap& smap) {
...
OdometryReading*
SensorStream::parseOdometry(std::istream& is, const OdometrySensor* osen) {
...
SensorStream::parseRange(std::istream& is, const RangeSensor* rs) {
	...
	if (rs->newFormat) {
		string laser_tv, laser_rv, forward_safety_dist, side_safty_dist,
				turn_axis;
		is >> laser_tv >> laser_rv >> forward_safety_dist >> side_safty_dist
				>> turn_axis;
	}
// 	else {
// 	  double a,b,c;
...
//LogSensorStream
LogSensorStream::LogSensorStream(const SensorMap& sensorMap,
		const SensorLog* log) :
...
SensorStream&
LogSensorStream::operator >>(const SensorReading*& rd) {
	rd = *m_cursor;
	m_cursor++;
	return *this;
}
...
//InputSensorStream
InputSensorStream::InputSensorStream(const SensorMap& sensorMap,
		std::istream& is) :
...
SensorStream&
InputSensorStream::operator >>(const SensorReading*& reading) {
