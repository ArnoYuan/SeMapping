...
istream&
CarmenConfiguration::load(istream& is) {
	clear();
	...
	while (is) {
		...
		if (qualifier == "ROBOTLASER1") {
			...
			lis >> laser_type >> start_angle >> field_of_view
					>> angular_resolution >> maximum_range >> accuracy
					>> remission_mode >> beams;
			rlaser1 = true;
		}
		...
		if (key != end()) {
			...
			if ((soff.size() / 3 < sonar_num)) {
				...
				cout << __PRETTY_FUNCTION__ << ": Ok " << soff.size()
						<< " parameters for defining the sonar offsets of "
						<< sonar_num << " devices" << endl;
			}
