  ...
  SensorMap CarmenConfiguration::computeSensorMap() const
  {
    ...
    if(key != end() && key->second.front() == "on")
    {
      ...
      if(key != end())
      {
        ...
        {
          cout << __PRETTY_FUNCTION__ << ": Error " << soff.size() << " parameters for defining the sonar offsets" << " while the specified number of sonars requires " << sonar_num * 3 << " parameters at least" << endl;
        }
        else
        {
