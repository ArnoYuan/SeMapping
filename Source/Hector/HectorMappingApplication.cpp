...
HectorMappingApplication::HectorMappingApplication() {
	...
	map_tf_srv = new NS_Service::Server<NS_ServiceType::ServiceTransform>(
			"ODOM_MAP_TF",
			boost::bind(&HectorMappingApplication::mapTransformService, this,
					_1));

	map_srv = new NS_Service::Server<NS_ServiceType::ServiceMap>("MAP",
			boost::bind(&HectorMappingApplication::mapService, this, _1));

	laser_sub = new NS_DataSet::Subscriber<NS_DataType::LaserScan>("LASER_SCAN",
			boost::bind(&HectorMappingApplication::laserDataCallback, this,
					_1));
...
void HectorMappingApplication::laserDataCallback(
		NS_DataType::LaserScan& laser) {
	laser_count++;
///////////////////////////////////////////////////////////////////////////
	/*
...
////////////////////////////////////////////////////////////////////////////
	...
	poseInfoContainer_.update(slamProcessor->getLastScanMatchPose(),
			slamProcessor->getLastScanMatchCovariance(), laser.header.stamp,
			"map");
...
void HectorMappingApplication::mapService(NS_ServiceType::ServiceMap& srv_map) {
...
void HectorMappingApplication::mapTransformService(
		NS_ServiceType::ServiceTransform& transform) {
	boost::mutex::scoped_lock map_mutex(map_to_odom_lock);
	transformTFToMsg(map_to_odom, transform.transform);
}

void HectorMappingApplication::getMapInfo(NS_DataType::OccupancyGrid& map,
...
HectorMappingApplication::getUnitVectors(double angle_min, double angle_max,
		double angle_increment, unsigned int length) {
...
void HectorMappingApplication::projectLaser(
...
bool HectorMappingApplication::pointsToDataContainer(
	...
	for (size_t i = 0; i < size; ++i) {
		...
		if ((dist_sqr > sqr_laser_min_dist_)
				&& (dist_sqr < sqr_laser_max_dist_)) {
			...
			if (pointPosLaserFrameZ > laser_z_min_value_
				...
				dataContainer.add(
						Eigen::Vector2f(pointPosBaseFrame.x(),
								pointPosBaseFrame.y()) * scaleToMap);
...
bool HectorMappingApplication::laserScanToDataContainer(
...
void HectorMappingApplication::updateMap(NS_DataType::OccupancyGrid& map,
		const NS_HectorMapping::GridMap& gridMap,
...
void HectorMappingApplication::updateMapLoop(double frequency) {
...
void HectorMappingApplication::loadParameters() {
...
void HectorMappingApplication::run() {
	console.message("hector is running!");

	loadParameters();

	slamProcessor = new HectorSlamProcessor(static_cast<float>(map_resolution_),
			map_size_, map_size_, Eigen::Vector2f(map_start_x_, map_start_y_),
			map_multi_res_levels_, hectorDrawings, debugInfoProvider);
...
void HectorMappingApplication::quit() {
