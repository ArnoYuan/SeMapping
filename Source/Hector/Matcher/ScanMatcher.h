...
public:
	...
	ScanMatcher(DrawInterface* drawInterfaceIn = 0,
	...
	Eigen::Vector3f matchData(const Eigen::Vector3f& beginEstimateWorld,
		...
		if (dataContainer.getSize() != 0) {
			...
			for (int i = 0; i < numIter; ++i) {
				...
				estimateTransformationLogLh(estimate, gridMapUtil,
						dataContainer);
				...
				if (drawInterface) {
					...
					drawInterface->setColor(static_cast<float>(i) * invNumIterf,
							0.0f, 0.0f);
			...
			estimate[2] = NS_HectorMapping::normalizeAngle(estimate[2]);
	...
	void drawScan(const Eigen::Vector3f& pose,
		...
		for (int i = 0; i < size; ++i) {
			...
			drawInterface->drawPoint(
					gridMapUtil.getWorldCoordsPoint(transform * currPoint));
		}
	}
