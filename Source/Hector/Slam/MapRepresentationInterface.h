#ifndef _HECTOR_MAP_REPRESENTATION_INTERFACE_H_
#define _HECTOR_MAP_REPRESENTATION_INTERFACE_H_
...
public:
	...
	virtual float
	getScaleToMap() const = 0;

	virtual int
	getMapLevels() const = 0;
	virtual const GridMap&
	getGridMap(int mapLevel = 0) const = 0;

	virtual void
	addMapMutex(int i, MapLockerInterface* mapMutex) = 0;
	...
	virtual void
	setUpdateFactorFree(float free_factor) = 0;
	virtual void
	setUpdateFactorOccupied(float occupied_factor) = 0;
};
