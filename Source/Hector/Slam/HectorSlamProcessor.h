...
public:
	...
	HectorSlamProcessor(float mapResolution, int mapSizeX, int mapSizeY,
		...
		mapRep = new MapRepMultiMap(mapResolution, mapSizeX, mapSizeY,
				multi_res_size, startCoords, drawInterfaceIn, debugInterfaceIn);
	...
	void update(const DataContainer& dataContainer,
		...
		if (!map_without_matching) {
			newPoseEstimateWorld = (mapRep->matchData(poseHintWorld,
					dataContainer, lastScanMatchCov));
		} else {
		...
		//std::cout << "\n1";
		//std::cout << "\n" << lastScanMatchPose << "\n";
		if (NS_HectorMapping::poseDifferenceLargerThan(newPoseEstimateWorld,
				lastMapUpdatePose, paramMinDistanceDiffForMapUpdate,
				paramMinAngleDiffForMapUpdate) || map_without_matching) {
		...
		if (drawInterface) {
			...
			drawInterface->drawPoint(
					gridMapRef.getWorldCoords(
							(gridMapRef.getMapDimensions().array() - 1).cast<
									float>()));
