#ifndef _HECTOR_MAP_REP_MULTIMAP_H_
#define _HECTOR_MAP_REP_MULTIMAP_H_

#include "../Map/GridMap.h"
#include "../Map/OccGridMapUtilConfig.h"
#include "../Matcher/ScanMatcher.h"
#include "../Slam/MapProcContainer.h"
...
public:
	MapRepMultiMap(float mapResolution, int mapSizeX, int mapSizeY,
		...
		for (unsigned int i = 0; i < numDepth; ++i) {
			std::cout << "HectorSM map lvl " << i << ": cellLength: "
					<< mapResolution << " res x:" << resolution.x()
					<< " res y: " << resolution.y() << "\n";
			GridMap* gridMap = new NS_HectorMapping::GridMap(mapResolution,
					resolution, Eigen::Vector2f(mid_offset_x, mid_offset_y));
			OccGridMapUtilConfig<GridMap>* gridMapUtil =
					new OccGridMapUtilConfig<GridMap>(gridMap);
			ScanMatcher<OccGridMapUtilConfig<GridMap> >* scanMatcher =
					new ScanMatcher<OccGridMapUtilConfig<GridMap> >(
	...
	virtual float getScaleToMap() const {
		return mapContainer[0].getScaleToMap();
	}
	;
	...
	getGridMap(int mapLevel) const {
		return mapContainer[mapLevel].getGridMap();
	}
	;
	...
	virtual Eigen::Vector3f matchData(const Eigen::Vector3f& beginEstimateWorld,
		...
		for (int index = size - 1; index >= 0; --index) {
			//std::cout << " m " << i;
			if (index == 0) {
				tmp = (mapContainer[index].matchData(tmp, dataContainer,
						covMatrix, 5));
			} else {
				dataContainers[index - 1].setFrom(dataContainer,
						static_cast<float>(1.0
								/ pow(2.0, static_cast<double>(index))));
				tmp = (mapContainer[index].matchData(tmp,
						dataContainers[index - 1], covMatrix, 3));
			}
		}
		return tmp;
	}
	...
	virtual void updateByScan(const DataContainer& dataContainer,
		...
		for (unsigned int i = 0; i < size; ++i) {
			...
			if (i == 0) {
				...
				mapContainer[i].updateByScan(dataContainers[i - 1],
						robotPoseWorld);
			}
		}
		//std::cout << "\n";
