...
public:
	...
	OccGridMapBase(float mapResolution, const Eigen::Vector2i& size,
			const Eigen::Vector2f& offset) :
			GridMapBase<ConcreteCellType>(mapResolution, size, offset), currUpdateIndex(
					0), currMarkOccIndex(-1), currMarkFreeIndex(-1) {
	}
	...
	void updateByScan(const DataContainer& dataContainer,
		...
		Eigen::Vector2i scanBeginMapi(scanBeginMapf[0] + 0.5f,
				scanBeginMapf[1] + 0.5f);
	...
	inline void updateLineBresenhami(const Eigen::Vector2i& beginMap,
			const Eigen::Vector2i& endMap, unsigned int max_length = UINT_MAX) {

		int x0 = beginMap[0];
		int y0 = beginMap[1];

		//check if beam start point is inside map, cancel update if this is not the case
		if ((x0 < 0) || (x0 >= this->getSizeX()) || (y0 < 0)
				|| (y0 >= this->getSizeY())) {
			return;
		}
		...
		int offset_dx = NS_HectorMapping::sign(dx);
		int offset_dy = NS_HectorMapping::sign(dy) * this->sizeX;
		...
		if (abs_dx >= abs_dy) {
			...
			bresenham2D(abs_dx, abs_dy, error_y, offset_dx, offset_dy,
					startOffset);
		} else {
			//otherwise y is dominant
			int error_x = abs_dy / 2;
			bresenham2D(abs_dy, abs_dx, error_x, offset_dy, offset_dx,
					startOffset);
		}
