#ifndef _OccGridMapUtil_h_
#define _OccGridMapUtil_h_
...
public:
	...
	void getCompleteHessianDerivs(const Eigen::Vector3f& pose,
			const DataContainer& dataPoints, Eigen::Matrix3f& H,
			Eigen::Vector3f& dTr) {
		...
		for (int i = 0; i < size; ++i) {
			...
			H(0, 0) += NS_HectorMapping::sqr(transformedPointData[1]);
			H(1, 1) += NS_HectorMapping::sqr(transformedPointData[2]);
			H(2, 2) += NS_HectorMapping::sqr(rotDeriv);
	...
	Eigen::Matrix3f getCovarianceForPose(const Eigen::Vector3f& mapPose,
		...
		sigmaPoints.block<3, 1>(0, 0) = Eigen::Vector3f(x + deltaTransX, y,
				ang);
		sigmaPoints.block<3, 1>(0, 1) = Eigen::Vector3f(x - deltaTransX, y,
				ang);
		sigmaPoints.block<3, 1>(0, 2) = Eigen::Vector3f(x, y + deltaTransY,
				ang);
		sigmaPoints.block<3, 1>(0, 3) = Eigen::Vector3f(x, y - deltaTransY,
				ang);
		...
		likelihoods[0] = getLikelihoodForState(
				Eigen::Vector3f(x + deltaTransX, y, ang), dataPoints);
		likelihoods[1] = getLikelihoodForState(
				Eigen::Vector3f(x - deltaTransX, y, ang), dataPoints);
		likelihoods[2] = getLikelihoodForState(
				Eigen::Vector3f(x, y + deltaTransY, ang), dataPoints);
		likelihoods[3] = getLikelihoodForState(
				Eigen::Vector3f(x, y - deltaTransY, ang), dataPoints);
		likelihoods[4] = getLikelihoodForState(
				Eigen::Vector3f(x, y, ang + deltaAng), dataPoints);
		likelihoods[5] = getLikelihoodForState(
				Eigen::Vector3f(x, y, ang - deltaAng), dataPoints);
		likelihoods[6] = getLikelihoodForState(Eigen::Vector3f(x, y, ang),
				dataPoints);
		...
		for (int i = 0; i < 7; ++i) {
			Eigen::Vector3f sigPointMinusMean(
					sigmaPoints.block<3, 1>(0, i) - mean);
	...
	Eigen::Matrix3f getCovMatrixWorldCoords(const Eigen::Matrix3f& covMatMap) {
		...
		float scaleTransSq = NS_HectorMapping::sqr(scaleTrans);
	...
	float getResidualForState(const Eigen::Vector3f& state,
		...
		for (int i = 0; i < size; i += stepSize) {
			...
			float funval = 1.0f
					- interpMapValue(transform * dataPoints.getVecEntry(i));
			residual += funval;
		}
	...
	float getUnfilteredGridPoint(Eigen::Vector2i& gridCoords) const {
		return (concreteGridMap->getGridProbabilityMap(gridCoords.x(),
				gridCoords.y()));
	}
	...
	float interpMapValue(const Eigen::Vector2f& coords) {
		...
		return ((intensities[0] * xFacInv + intensities[1] * factors[0])
				* (yFacInv))
				+ ((intensities[2] * xFacInv + intensities[3] * factors[0])
						* (factors[1]));
	...
	Eigen::Vector3f interpMapValueWithDerivatives(
		...
		return Eigen::Vector3f(
				((intensities[0] * xFacInv + intensities[1] * factors[0])
						* (yFacInv))
						+ ((intensities[2] * xFacInv
								+ intensities[3] * factors[0]) * (factors[1])),
				-((dx1 * xFacInv) + (dx2 * factors[0])),
				-((dy1 * yFacInv) + (dy2 * factors[1])));
	}

	Eigen::Affine2f getTransformForState(
	...
	Eigen::Translation2f getTranslationForState(
			const Eigen::Vector3f& transVector) const {
		return Eigen::Translation2f(transVector[0], transVector[1]);
	}
