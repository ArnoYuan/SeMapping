...
public:
	...
	bool hasGridValue(int x, int y) const {
		return (x >= 0) && (y >= 0) && (x < this->getSizeX())
				&& (y < this->getSizeY());
	}

	const Eigen::Vector2i&
	getMapDimensions() const {
		return mapDimensionProperties.getMapDimensions();
	}
	;
	int getSizeX() const {
		return mapDimensionProperties.getSizeX();
	}
	;
	int getSizeY() const {
		return mapDimensionProperties.getSizeY();
	}
	;
	...
	getMapDimProperties() const {
		return mapDimensionProperties;
	}
	;
	 ...
	 */
	GridMapBase&
	operator=(const GridMapBase& other) {
		if (!(this->mapDimensionProperties == other.mapDimensionProperties)) {
			this->setMapGridSize(
					other.mapDimensionProperties.getMapDimensions());
		}
		...
		memcpy(this->mapArray, other.mapArray,
				sizeX * sizeY * concreteCellSize);
	 ...
	 */
	inline Eigen::Vector2f getWorldCoords(
	 ...
	 */
	inline Eigen::Vector2f getMapCoords(
	...
	void setDimensionProperties(const Eigen::Vector2f& topLeftOffsetIn,
			const Eigen::Vector2i& mapDimensionsIn, float cellLengthIn) {
		setDimensionProperties(
				MapDimensionProperties(topLeftOffsetIn, mapDimensionsIn,
						cellLengthIn));
	}
